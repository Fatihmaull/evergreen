/**
 * The scan panel: one contract ID, optional data keys, then the verdict, the
 ...
 *
 * Shared by `/dashboard` and `/dashboard/scanner` so there is one
 * implementation rather than two. The spec settles scan-and-status as "one
 * combined surface"; twelve routes keep that intent only while `/dashboard`
 * alone is sufficient, which means the scan has to live on it.
 *
...
import { approxXlm, esc, formatCount } from './format';
import { blastRadiusPanel, entryTable, jsonPanel, verdict, wireCopyButtons } from './ui';
    ...
    panel.innerHTML = `
      ...
      ${
        quote.excluded.length > 0
          ? `<div class="excluded"><p class="small"><strong>${quote.excluded.length} entr${quote.excluded.length === 1 ? 'y is' : 'ies are'} not in this estimate.</strong>
               Core leaves them out rather than pricing an operation that cannot be performed:</p>
             <ul class="small">${quote.excluded
               .map(
                 (x) =>
                   `<li><span class="mono">${esc(x.entryKey.slice(0, 10))}…</span> — ${esc(x.detail)}</li>`,
               )
               .join('')}</ul></div>`
          : ''
      }`;
...
/** Wire the form. Absent on a page that does not render one, so this is a no-op there. */
export function mountScanPanel(): void {
  const form = document.getElementById('scan-form');
  if (form === null) return;
  form.addEventListener('submit', (e) => void runScan(e));
