...
export function render(ctx) {
  ...
  return `<section class="card card-pad stack">
    <h2 class="section">Usage and flags</h2>
    <div class="terminal">${esc(ctx.cli.help)}</div>
    <p class="small muted">Source: <span class="mono">packages/cli/src/command.ts</span> — this page fails to build when that help text moves.</p>
  </section>
  <section class="card card-pad stack">
    <h2 class="section">Exit codes</h2>
    <div class="table-wrap"><table>
      <thead><tr><th scope="col">Exit</th><th scope="col">Meaning</th></tr></thead>
      <tbody>${exitRows}</tbody>
    </table></div>
    <p class="small muted">Precedence 2 &gt; 3 &gt; 1 &gt; 0, from <span class="mono">packages/cli/src/scan.ts</span>. A degraded read still exits 3; blast radius changes severity, not the exit category. Exit status never authorizes a transaction.</p>
  </section>
  <section class="card card-pad stack">
    <h2 class="section">JSON shape</h2>
    <p class="muted">
