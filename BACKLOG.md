...
- [~] **W2-D8-03** (R) Scan all entry types for a contract (instance, code/wasm, persistent, temporary). **2026-09-09 published for shared review in [PR #60](https://github.com/Fatihmaull/evergreen/pull/60):** `feat/W2-D8-03-scan-entry-types`, base main `321656b`. Instance/code discovery plus explicit data keys; coverage is known-keys, never full enumeration. [ADR-006](docs/adr/ADR-006-scan-health-exit-codes.md) proposes exit 3 for incomplete information and `--no-data-keys` as a caller assertion, with precedence 2 > 3 > 1 > 0. `pnpm check` passed 143 offline tests. [Sep 9 read-only proof](docs/evidence/2026-09-09-scan-coverage/README.md): A without supplied data keys returns 3 despite healthy instance/code; original [four-type proof](docs/evidence/2026-09-08-scan-entry-types/README.md) remains unchanged. [Issue #44](https://github.com/Fatihmaull/evergreen/issues/44#issuecomment-5589189730) tracks Fatih's finding. No cross-contract dedup or spending path is included. Sep 9 follow-up records Fatih's milestone/assertion-misuse consequences and A-extension reproduction caveat; ADR-006 remains Proposed pending final acceptance.
- [x] **W2-D8-04** (R) **Deduplicate by ledger key.** Scanning N contracts that share a Wasm surfaces the same `ContractCode` entry N times. `ScanResult` must carry unique entries with the set of contracts each one serves — that set is what severity and reporting both need downstream. **2026-09-09 reviewed and published in [PR #63](https://github.com/Fatihmaull/evergreen/pull/63), based on #60; Fatih review requested:** `scanContracts` deduplicates before RPC, preserves per-contract coverage and partial diagnostics; the single-contract wrapper and legacy consumer dedup are covered. `pnpm check` passed 170 offline tests. [Read-only B/C proof](docs/evidence/2026-09-09-scan-dedup/README.md) at ledger 4,586,511: input B/C/B → two consumers, two instances, one shared Wasm, entry-request sizes 2 + 1. No transaction or new CLI batch syntax.
...
- [ ] **W2-D11-01** (R) `evergreen extend <contract-id> --ledgers N` — manual `extendTTL` submission, signed locally by the developer key. **This is the same signing path Stage 1 ships** (`W3-D16-01`) and the one the README quickstart teaches — the capped policy signer is the Stage 2 *hardened* path (`W3-D19`), not a later replacement for it.
