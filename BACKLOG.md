...
- [~] **W1-D4-13** (R) ⏱️ **START THIS FIRST — it is mostly waiting, not working.** ~57 minutes of elapsed time, only a few minutes of actual effort: seed the entry, then poll. **Run it in the background alongside `W1-D4-01/02/03`**, which are hands-on and will fill the wait. Do **not** queue it behind them. `W2-D8-01` (Thu) cannot be written correctly until this lands, so picking it up last turns an hour of waiting into a day of slippage on the week's first real math task. **Observe the exact TTL boundary ledger — before `W2-D8-01`'s math is written, not after.** Seed a temporary entry (688-ledger floor, ~57 min, so the cycle is cheap), poll `getLedgerEntries`, and record the **exact** ledger at which the read starts failing. Confirms or refutes the documented inclusive boundary and the `remainingLedgers == 0` trap in `docs/SOROBAN-PRIMER.md`. *Partial evidence already exists — B and C's temporary entries were absent ~1,300 ledgers past expiry, which is consistent but only bounds it.* Record what the docs claim **and** what you saw; **if they disagree, the observation wins and it gets escalated.** Same shape as `W1-D4-06`.
