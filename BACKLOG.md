...
- [~] **W1-D6-04** (R) *(started 2026-09-08 — spike branch `chore/W1-D6-04-persistence-spike` pushed with a store, a contender harness and tests; reassigned from F — issue #30 assigned it to Rakha and the backlog had not caught up)* Decide and document the persistence choice for bump history (flat JSON file vs SQLite vs hosted KV) in ADR-003 — the dashboard and engine both read it, so it can't be decided ad hoc in Week 3.
