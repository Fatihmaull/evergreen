...
- [~] **W2-D10-01** (F) **PARTIALLY COMPLETE — exit-code semantics done in [PR #66](https://github.com/Fatihmaull/evergreen/pull/66) (S1, ADR-006 amendment); display work remains (S2).** The split is structural, not careless: accepting an ADR that changes behaviour necessarily lands the code for that behaviour, so S1 could not amend the exit-code scheme without touching `exitCodeFor`. **#66 covered:** exit 3 redefined as *a degraded scan*, not an undeclared scope; `requireDeclaredScope` made opt-in; the CLI/ADR docs for it. **Still open, and the actual body of this task:** `--json` vs human-readable output, coloured health states, blast-radius severity, shared-entry disclosure, and a `--help` a stranger can follow. Original task text: CLI UX: `--json` vs human-readable output, colored TTL health states (healthy / warning / critical), `--help` that a stranger can follow.
