...
- [~] **W1-D6-04** (R) Decide and document persistence/atomic locking in ADR-003. **Started 2026-09-08; local spike ready for review:** eight PostgreSQL checks passed; controlled database-unavailable refusal verified; local Workers SDK Testnet reads passed. [Evidence](docs/evidence/2026-09-08-persistence-spike/README.md), [ADR proposal](docs/adr/ADR-003-toolchain-hosting-persistence.md). Actions + Node + PostgreSQL remains proposed; runtime agreement, Neon/Supabase selection and hosted validation remain open in [Issue #30](https://github.com/Fatihmaull/evergreen/issues/30). Dashboard hosting is separately settled on Pages. Reassigned from F to R per [handoff #39](https://github.com/Fatihmaull/evergreen/issues/39#issuecomment-5579179171); primary branch `chore/W1-D6-04-persistence-spike` is pushed with the store, contender harness, tests and evidence. No production engine or live bump.
