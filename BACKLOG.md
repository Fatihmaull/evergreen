...
- [ ] **W3-D18-02d** (F) 🔴 **Extend the shared `ContractCode` entry — only after Sep 25, and not before.** A, B and C were all built from the same Wasm (`c7e55f0a…98bfb`, verified by fetching and hashing each contract on 2026-09-09), so they share **one** code ledger entry, currently live until **5,290,829 ≈ 2026-10-20 06:38 UTC**. A contract is only as alive as its code entry, so on that date **all three become unusable at once** — including the A that `W1-D7-08` just extended to December, and every artifact that depends on reading A. It was left alone on Sep 9 because `SETUP` forbids touching it while the decay proofs are live. **Once `W3-D18-02b` and `W3-D18-02c` are captured, that constraint is gone and this becomes free.** Extend to at least Dec 2026, matching A's other entries. *Do not do this earlier as a convenience — the whole reason it is a separate task is that the shared entry cannot be extended for one contract only.*
