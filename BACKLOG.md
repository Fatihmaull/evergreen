...
- [!] **W1-D5-01** (F) 🔴 **DEADLINE Sep 16, not Week 4.** The package name appears in the README quickstart, `docs/ONBOARDING.md` and the demo script — and **`W2-D14-03` captures CLI screenshots on Sep 16 as evidence snapshot #2.** If those screenshots show `npx evergreen` while the published name is `evergreen-soroban`, the evidence is wrong and gets retaken during publish week, which is the expensive version of the problem this task exists to prevent. **BLOCKED — `evergreen` is taken on npm.** Squatted by an abandoned 2016 package (MongoDB build-platform client, last published 2016-04-28), so `packages/cli`'s declared name and the README's `npx evergreen` are both unpublishable as written. Checked 2026-09-07: `evergreen-soroban`, `soroban-evergreen`, `stellar-evergreen` and `@evergreen-soroban/*` are all free; the `@evergreen` scope could not be confirmed unauthenticated (npm org page 403). **The command name is recoverable even if the package name is not** — a package published as `evergreen-soroban` can still declare `"bin": {"evergreen": …}`, so `npm i -g` gives users `evergreen scan`. Only `npx <name>` and the install line change. Needs Fatih's npm account. Reserve the scope/package names (`evergreen` CLI + core) — confirm availability *now*, not in Week 4.
- [~] **W1-D5-02** (F) Dashboard hosting account + empty project deployed. **Deploy target prepared**: `apps/dashboard/public/index.html` is a static placeholder that explains what the page is and that the real dashboard lands W4 — deliberately framework-free, since the framework choice is still deferred and a hello-world should prove the *pipeline*, not commit the stack. **Recommendation Cloudflare Pages** — not on hosting merit (the P0 dashboard is static, so all three are identical) but because the same signup provides the account needed to settle the Workers/Stellar-SDK question blocking ADR-003 Part 2. Reasoning in ADR-003. **Needs Fatih: create the account and connect the repo.**
...
- [x] **W1-D6-04** (R) ✅ **DECIDED 2026-09-08 — ADR-003 Part 2 accepted.** Runtime: Actions cron + Node 24, Workers dropped for W1. Persistence: PostgreSQL on **Neon, adopted Week 4 — deliberately not before the Sep 20 proof.** Neon's free tier bills a non-disableable 300s idle window per run: 240 CU-hours/month against a 100-hour allowance, exhausting on **day 12.5 = Sep 20** at a 1.0 CU ceiling, with no free-plan warning and no reset until after the deadline. Deeper reason: the lock guards a case that cannot currently occur (`concurrency:` + `timeout-minutes` already prevent overlap), and the ledger is already the durable atomic store. Shipping `W2-D10-04` instead. *(started 2026-09-08 — spike branch `chore/W1-D6-04-persistence-spike` pushed with a store, a contender harness and tests; reassigned from F — issue #30 assigned it to Rakha and the backlog had not caught up)* Decide and document the persistence choice for bump history (flat JSON file vs SQLite vs hosted KV) in ADR-003 — the dashboard and engine both read it, so it can't be decided ad hoc in Week 3.
