...
- [x] **W1-D6-02** (R) Write `docs/ARCHITECTURE.md` data-flow section against those types (RPC → core → {CLI, engine, dashboard}). **2026-09-08: complete on `docs/W1-D6-02-architecture-flow`; final local review passed, awaiting PR publication.** Three diagrams distinguish implemented scan, module dependencies and planned engine flow; shared-entry example, payer/signing/outcome semantics and W3/W4 persistence boundaries documented. `pnpm check` passed 63 offline tests plus typecheck/lint/formatting. [#32](https://github.com/Fatihmaull/evergreen/issues/32); documentation only, no runtime or type changes.
