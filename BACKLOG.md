...
- [x] **W1-D7-04** (S) ✅ **Accepted 2026-09-09 by merging #57.** Adjust W2–W4 tasks if W1 revealed anything (e.g. RPC quirks, tooling surprises). Record changes in STATUS.md. *Carried forward: the `W3-D15-02b` temporary-entry split (reporting is W2 and not open; auto-bumping stays W3), and the guinea-pig A Sep 16 expiry noted below, which was found on the day W1 closed and changes `W3-D17-04`.*
...
> 🔴 **Carried into W2, found on Sep 9 and not previously recorded anywhere:** guinea-pig **A**'s instance, persistent and temporary entries all expire **Wed Sep 16 ~19:27 UTC** (ends at ledger 4,712,648; its code entry survives to Oct 20). A is the working contract, not a decay subject — `W3-D17-04` requires bumping it *before Fri Sep 18*, by which time it is archived and `extendTTL` no longer applies. See `docs/STATUS.md` § Right now.
