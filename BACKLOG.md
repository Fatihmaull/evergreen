...
**Sequence position is ahead:** on calendar day 3, Day 3 and Fatih's initial Day 4 research/deploy tasks are complete. Rakha's `W1-D4-03/14` are now done; `01/02` are ready locally but await Fatih's machine/account confirmation. `W1-D4-13` is observing a fresh temporary entry's exact boundary. See STATUS for the latest result.
...
- [x] **W1-D4-14** (R) **Exclude generated Rust build output from repository checks.** Added 2026-09-05 during local setup ([Issue #17](https://github.com/Fatihmaull/evergreen/issues/17)): ESLint traversed a disappearing `target/debug/deps/rustc*` directory while `cargo test` ran and failed with ENOENT; Prettier also lacks a target exclusion. Ignore build output in both tools and verify `pnpm check` with Rust artifacts present.
