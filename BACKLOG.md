...
**Absorbed by running ahead (W1, already done):** `W1-D4-00` guinea-pig contract · `W1-D4-07` guinea-pig C · `W1-D4-08`/`09` drift script and recurring check · `W1-D4-10` shared-code propagation · `W1-D4-11` dual-channel sync.
...
**Sequence position is ahead:** Day 3 and the initial Day 4 research/deploy tasks are complete. Local setup for `W1-D4-01/02/03` is verified; `01/02` still await current tooling and everyday-account confirmation from the second developer. `W1-D4-03/14` are complete locally. The separate `W1-D4-13` boundary experiment remains in progress and paused. See STATUS for details.
...
- [~] **W1-D7-04** (S) Adjust W2–W4 tasks if W1 revealed anything (e.g. RPC quirks, tooling surprises). ADR-005 and carry-forward accepted in #58; shared closeout review remains open in [#44](https://github.com/Fatihmaull/evergreen/issues/44), with the recovered W1 report published in [#57](https://github.com/Fatihmaull/evergreen/pull/57).
...
- [ ] **W2-D8-01** (F) TTL math module: remaining ledgers → projected archive ledger → projected archive **date** using network ledger-close cadence.
  - **First, settle the boundary convention** — is an entry live *at* `liveUntilLedgerSeq`, or is that the first dead ledger? `docs/SOROBAN-PRIMER.md` flags this as open and it is the whole arithmetic content of this task. Verify against the official docs *and* observe it directly: a temporary entry expires in ~57 minutes, so the boundary is cheap to watch rather than reason about. Record the answer in the primer.
