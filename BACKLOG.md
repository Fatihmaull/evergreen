...
- [~] **W1-D5-04** (R) Email provider account + a test email successfully sent from code. **Script prepared**: `scripts/send-test-email.mjs`, guarded so it refuses to run without `EMAIL_API_KEY` and `EVERGREEN_ALERT_TO` rather than failing obscurely. Uses Resend's sandbox sender, which needs no domain verification and delivers only to the key owner — sufficient for a connectivity test, and our alerts go to two people who own their own addresses. **Needs Fatih: create the account, put the key in `.env`, run the script.** Delete the script once `EmailChannel` lands at `W3-D17-01`; parallel tooling that outlives its purpose is how a repo accumulates things nobody dares remove.
