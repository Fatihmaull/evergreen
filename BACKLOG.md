...
- [~] **W1-D5-03** (R) Scheduler platform decided and a trivial cron job running end-to-end (GitHub Actions cron vs Cloudflare Workers cron — see ADR-001). Prove it can run on schedule and log output before we trust it with bumps.
  - **2026-09-06 progress:** GitHub Actions + Node 24 selected for the initial smoke test. SDK 17.0.1 reads verified locally; script, workflow, and nine offline regression tests prepared on `chore/W1-D5-03-scheduler-smoke`; `pnpm check` passes. Published for review in [PR #24](https://github.com/Fatihmaull/evergreen/pull/24), tracking [Issue #23](https://github.com/Fatihmaull/evergreen/issues/23); merge and runtime proof remain pending. Keep `[~]` until the workflow reaches `main` and an actual `schedule` event succeeds with saved run logs. See ADR-003 for the decision and remaining proof.
