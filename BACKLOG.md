...
- [ ] **W2-D8-01** (F) TTL math module: remaining ledgers → projected archive ledger → projected archive **date** using network ledger-close cadence.
  ...
  - **Mind the vocabulary.** Temporary entries are *deleted*, not archived, so a type named `projectedArchiveDate` bakes the wrong word into a contract every module speaks. Prefer end-of-life naming that carries the distinction.
