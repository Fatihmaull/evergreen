...
- [ ] **W1-D6-01** (R) Define shared TypeScript types in `packages/shared-types`: `ContractRef`, `LedgerEntryTTL`, `ScanResult`, `RentEstimate`, `BumpDecision`, `BumpRecord`, `NotificationChannel`, `EvergreenConfig`, `Signer`. These are the seams every later task codes against — and per `AGENTS.md` they must not churn mid-week, so get them right today.
- [ ] **W1-D6-01c** (R) **Name the TTL end-of-life fields carefully — this vocabulary enters every module.** Not `projectedArchiveDate`: *archive* is false for temporary entries (deleted, unrestorable) and *date* invites storing wall-clock where the truth is a ledger. Keep **when** and **what happens** separate — `endsAtLedger: number` plus `endBehavior: 'archived' | 'deleted'` — and derive any wall-clock estimate at the display edge without storing it. Also settle the boundary semantics from `W1-D4-13` here, since `endsAtLedger` means nothing until "live *at* that ledger, or after it?" is answered.
