...
- [ ] **W1-D7-07** (S) **Check the four deliberately duplicated statements still agree.** `docs/ONBOARDING.md` and `AGENTS.md` both carry: the non-custodial rationale, the definition of done, the dry-run default, and the evidence rule. That duplication is justified — orientation needs enough to act safely before the manual is read — but **an unchecked deliberate duplication becomes an unnoticed accidental one within weeks.** Four strings, ten seconds when it is on a list, a silent divergence when it is not. Repeat at every week gate.
