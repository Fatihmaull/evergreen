...
- Optional `coverage` records `mode: 'known-keys'` and `dataKeysSuppliedByContract`, counting unique validated explicit data keys. It never asserts full storage enumeration. Omission by an older producer means coverage is unspecified, not complete.
