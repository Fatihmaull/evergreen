...
- **Stage 2 (planned hardened path, unavailable):** the proposed direct smart-account adapter does not constrain the native TTL fee-payer signature. Shared direction is pending; do not treat the config/type seam as an implemented signer. See [signer status and limits](../../docs/POLICY-SIGNER.md).
