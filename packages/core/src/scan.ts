 ...
 *
 * This slice reads instance entries only (`W1-D7-01`). Use `scanContracts` for code,
 * persistent and temporary entries. This legacy scan is deliberately
...
export async function scanInstances(
  ...
  const uniqueContracts = new Map<ContractId, ContractRef>();
  for (const contract of contracts) {
    const previous = uniqueContracts.get(contract.id);
    if (!previous || (previous.label === undefined && contract.label !== undefined)) {
