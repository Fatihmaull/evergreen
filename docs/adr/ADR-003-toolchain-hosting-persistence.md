...
- The first successful manual and scheduled run URLs, event types, commit SHAs, and full logs are captured. Publish the reviewed evidence so the record remains available independently of GitHub log retention.
...
- 2026-09-07: PR #24 merged; manual run `34110254224` and genuine scheduled run `34111732199` succeeded. Their full logs and metadata were captured and cross-checked. Runtime proof complete, evidence publication awaiting review; no change to the platform decision or the separate hosting/atomicity tasks.
