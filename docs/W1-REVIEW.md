...
|---|---|---|---|
| 2026-09-26 | Rakha / W4-D26-02 | Following setup docs to write an engine self-host guide | `docs/SETUP.md` lists `EVERGREEN_CONFIG_PATH`, but the shipped decide-only cron uses `EVERGREEN_CONFIG` and `engine:execute` accepts `--config`; neither command auto-loads `.env`. The existing engine README also does not distinguish the cron from bounded local live execution. Had to inspect the scripts and command help before publishing a runnable example. |
| 2026-09-14 | Rakha / D17-02 verification | Running the full gate after live email proof | Gitignored .evergreen JavaScript helpers are still scanned by ESLint. The one-shot live-email capture helper was archived outside the repo in /tmp; private result/intent files remain in .evergreen. Git ignore is not a lint exclusion, and gate rules were not changed. |
| 2026-09-14 | Rakha / D17-01 planning | Reusing merged notification templates and closing D16-01 | The interface already exists, but the backlog still asks to create it. D16 merge versus live-validation completion is unstated. The failed template claims no extension and safe retry even though a failed BumpRecord may mean confirmed transaction with unverified post-state; reproduced offline from origin/main. The plan separates completion gates and proposes neutral failure wording before email integration. |
| 2026-09-13 | Rakha / D16-01 planning | Reusing manual execution for the engine | The backlog says reuse the execution path but does not describe the single-contract planner's implicit instance selection, hard-coded manual record reason, missing public payer/cap config, or the pre-send durable-record boundary with D16-02. The local plan names those adapters and keeps across-run guarantees and live proof acceptance explicit. |
| 2026-09-12 | Rakha / D15-02 planning | Connecting the two health horizons | The backlog names existing HealthThresholds, but the assessment and engine consume one number; the Notion title still describes ledgers OR days. Config also does not yet define warning inheritance or reject unknown threshold keys. The local plan makes these choices explicit, preserves CLI behavior, and separates temporary opt-in from threshold reporting. |
| 2026-09-12 | Rakha / W3 startup | Refreshing before approved D15-01 implementation | Main advanced through #118: D15-01 was reassigned and merged, while STATUS still led with open #113 and Notion Notes described the old work. #114 also reversed B from save to intentional expiry. Read current GitHub ownership/content and ran four offline reproductions before considering any duplicate implementation. |
| 2026-09-12 | Rakha / W3 planning | Designing W3-D15-01 from the current core | D15 requires two health tiers and per-contract rules, but config still has one action field and assertLiveness one global threshold; config target and manual-helper delta also need an explicit adapter. The local W3-D15-01 plan proposes those mappings for review. STATUS still labels merged #105 as open; #104 corrects the handoff's explanation of the #102 review dismissal. GitHub/current source were needed to reconcile these facts. |
| 2026-09-11 | Rakha | Planning D11 live-proof evidence capture | The evidence checklist names explorer and before/after screenshots without a verified capture/export route for this harness. Chrome DevTools had no executable; in-app browser capture worked. Terminal screenshot capture and saving image artifacts must be confirmed before send, not discovered after a transaction. |
| 2026-09-12 | Rakha | Synchronizing the D11 live-proof plan with #103 | The issue correctly says the CLI adds N to current TTL, but later says a small N falls below current TTL and recommends N exceed the remainder. Read the merged resolveExtendTarget helper to resolve the contradiction; fee size alone cannot prove a no-op. |
| 2026-09-10 | Rakha | Implementing the D11 operation ceiling | The plan, primer and cost helper equated max_entry_ttl with the largest extendTo operation target. Stellar core validates extendTo against maxEntryTTL - 1 because the current ledger counts toward TTL. Logged before correcting the shared helper and related docs; simulation alone cannot establish transaction validity. |
| 2026-09-10 | Rakha | Checking toolchain instructions for D11 planning | SETUP's initial setup block still lists `pnpm typecheck && pnpm lint && pnpm test`; AGENTS requires full `pnpm check`. Used the canonical full check for this plan and recorded the discrepancy before resolving it; no setup-doc rewrite in this planning change. |
