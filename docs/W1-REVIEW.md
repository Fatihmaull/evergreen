...
|---|---|---|---|
| 2026-09-14 | Rakha / W3 Stage 1 implementation | Arming the A proof | The example names EVERGREEN_SIGNER_SECRET while its payer is the dev account. Local identity verification found the matching key under EVERGREEN_DEV_SECRET. The private proof config selects that existing variable; global env and payer stay unchanged. Screenshot export also required verifying the actual JPEG format before signing. |
