...
- **`ExtendFootprintTTLOp`** ("extendTTL") — extends the live-until ledger of entries in the read-only set of the transaction footprint. This is the write operation the auto-bump engine performs, and the *only* one the policy signer may authorize.
  ...
  - 🔴 **`simulateTransaction` does not validate `extendTo` against `max_entry_ttl`. It prices what you asked for, including requests the protocol cannot perform.** Measured 2026-09-10: asked for `extendTo: 4,000,000` against a 3,110,400 ceiling, simulation quoted **345,853 stroops** — about 52% more than the capped extension actually costs. It did not error and it did not clamp.
    So the failure is not only that a user receives a smaller extension than they asked for. **They are quoted for the thing that cannot happen.** Clamp to `max_entry_ttl` *before* quoting, and read that ceiling from the network — it is `stateArchivalSettings.max_entry_ttl` on the STATE_ARCHIVAL config entry (`AAAACAAAAAo=`), not a constant.
