...
**Last updated:** 2026-09-14 · Paket A reviewed locally; three fixes verified; awaiting publication
...
## Current — Paket A reviewed locally, awaiting publication

**Internal review completed:** [report](W3-PAKET-A-REVIEW.md). Three reproduced P2 findings fixed: watcher minted a new incident identity after a temporary API error, runtime builder could attribute stale/uncommitted output to HEAD, and campaign hashes could omit a runtime dependency. Fresh full `pnpm check` passed (731 Vitest + 95 Node = 826 tests). No blocking finding remains in the reviewed scope. Six captured scripts match commit 32670fe and 35 freshly built package files/metadata match the proof manifest; signed receipt/TTL/control verification still passes. No new email, timer, transaction, push or PR during review. Notion sync remains pending at publication.
...
**D18-02a remains In progress:** local scheduled execution is evidenced, but Fatih's Shared acceptance via #130 and publication remain. Existing GitHub cron stays decide-only; ephemeral live runners still lack durable pre-send intent storage. The same local campaign refuses reuse in the actual readiness gate and a fresh process. An additional timer-refusal test was not run because automatic approval review was at capacity; no second transaction was attempted.
...
Rakha explicitly authorized publication after internal review. [PR #122](https://github.com/Fatihmaull/evergreen/pull/122) publishes reviewed implementation 19f9275 and requests Fatih's review. It targets #121 (`feat/W3-D15-02-thresholds`, cdecdb8), which remains open alongside #120. Retarget #122 to main before merging/deleting #121's branch, then reconcile it with merged main and recheck CI. No merge or auto-merge was performed. Publication does not authorize live RPC/signing/submission. D15-02b remains a separate unpublished local draft. Notion read-back confirms W3-D16-01 is In progress/Rakha with the PR link, verified outcome and remaining Testnet gates. Presence validation found 149 registered IDs / 150 mirror rows, no missing or duplicate IDs; the sole extra is the known retired Dropped ~~W3-D18-02~~. The first Notion connection returned 404; the alternate configured connection succeeded. Initial PR CI caught a documentation-only retired-ID reference missing strikethrough; this publication follow-up corrects it without changing the gate or execution code.
...
## [S2] Blast radius does not reach the exit code — ADR-006 records the refusal

**2026-09-10, decided by Fatih.** `W2-D10-01` asked for blast-radius severity in "the colour states **and the exit code**." The exit-code half is **declined**. No new code, no re-meaning of an existing one. Magnitude belongs in output.

Three reasons, in Fatih's order of weight: exit codes signal **category, not magnitude** — "below threshold affecting twelve contracts" is the same category with a number attached; **every new code is a breaking change** to the Action's contract, since consumers write `if [ $? -eq 1 ]` and a `4` means they silently stop seeing the new case — a green pipeline where there should be a red one, which is this week's failure family authored straight into the one interface strangers depend on; and **CI has no degrees of failure** — a job fails or it does not, and how fast a human should move is learned by reading output, not by comparing a number to a constant.

If callers ever need to discriminate, the mechanism is an **input** — `--fail-on=critical` — not a wider output set. SOW 2, not this sprint.

**Recorded in ADR-006 as considered and declined, and the backlog line was corrected.** The line read as a requirement; leaving it would have had someone in Week 4 implement something an accepted ADR forbids. **A task that contradicts an ADR is a trap for whoever picks it up**, so it now states the refusal and links the reasoning.

**What shipped instead:** blast radius in the human output (already), and a `health` block in `--json` carrying `blastRadius`, `isShared`, the graded state and its reason per entry. **Additive** to `ScanResult` — every existing key untouched, pinned by a test — because `ScanResult` is ADR-005-accepted and read by the engine and dashboard. A test also asserts the shared and lone entries at identical TTL return the **same exit code**, so the declined design cannot creep back in unnoticed.

**Two rules added to the failure family at Fatih's request.**

*A green result about the wrong subject.* The recorded stale-CI rule covers a stale result for the right subject. It does not cover a result that is **fresh, green, and about someone else's work** — which is what happened when a PR number was inferred rather than read: `#73` was polled, reported two genuine passing checks, and belonged to the other session. That is the more dangerous shape, because staleness at least has a timestamp to interrogate while a fresh green offers nothing to be suspicious of. Its cousin, same day: `$?` after a pipe reports `tail`'s status, not the checker's.

*Guards get written by someone already thinking about the pattern — elsewhere.* Four instances this week, each inside the guard for the previous one: the constant-copy checker threw a stack trace on a rename; the liveness simulation restated the rule it tested; the lint rule fired on validation. The property, not the coincidence: **the guard is code, and code written to detect a pattern is written by someone currently attending to that pattern in a different place.** So exercise a new guard in its failing direction, degenerate cases included, before trusting it.

**And once more immediately.** Generalising the family heading from "six members" to "members" — the count had gone stale — broke an in-file anchor written an hour earlier. The anchor checker then reported it *still* broken after repair, because the checker stripped a leading hyphen GitHub keeps for an emoji-led heading. The instrument was wrong, not the link. Fixed the checker, re-verified: all anchors resolve. Same lesson as `$?` after a pipe, twice in one session.
